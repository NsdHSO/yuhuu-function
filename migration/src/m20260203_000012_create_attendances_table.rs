...
impl MigrationTrait for Migration {
        ...
        manager
            .create_table(
                Table::create()
                    ...
                    .col(ColumnDef::new(Attendances::Uuid).uuid().not_null().unique_key())
                    .col(ColumnDef::new(Attendances::UserId).big_integer().not_null())
                    .col(ColumnDef::new(Attendances::ServiceType).string().not_null())
                    .col(ColumnDef::new(Attendances::AttendanceDate).date().not_null())
                    .col(ColumnDef::new(Attendances::CheckInTime).timestamp())
                    .col(ColumnDef::new(Attendances::CheckOutTime).timestamp())
                    .col(ColumnDef::new(Attendances::Status).string().not_null().default("Present"))
                    .col(ColumnDef::new(Attendances::Notes).text())
                    .col(ColumnDef::new(Attendances::RecordedBy).big_integer())
                    .col(ColumnDef::new(Attendances::CreatedAt).timestamp().not_null().default(Expr::current_timestamp()))
                    .col(ColumnDef::new(Attendances::UpdatedAt).timestamp().not_null().default(Expr::current_timestamp()))
                    .foreign_key(
                        ForeignKey::create()
                            .name("fk_attendances_user_id")
                            .from((Alias::new("church"), Attendances::Table), Attendances::UserId)
                    ...
                    .foreign_key(
                        ForeignKey::create()
                            ...
                            .from((Alias::new("church"), Attendances::Table), Attendances::RecordedBy)
                            .to((Alias::new("church"), Alias::new("users")), Alias::new("id"))
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .drop_table(Table::drop().table((Alias::new("church"), Attendances::Table)).to_owned())
            .await
    }
}
